...
class DeepSpeedAgent:
    ...
    def __init__(self, model, args):
        ...
        self.load_parameters(self.args['load_path'], self.args['stage'])
    ...
    def evaluate_model(self, batch, output_file, current_step=0, pbar=None):
        self.ds_engine.module.eval()
        self.ds_engine.evaluate(batch, output_file)
        pbar.update(1)
        if self.args['local_rank'] == 0 and self.args['log_path'] and current_step % self.args['logging_step'] == 0:
            elapsed = pbar.format_dict['elapsed']
            rate = pbar.format_dict['rate']
    ...
    def save_model(self, path, current_step):
        ...
        """torch.save(self.ds_engine.module.state_dict(), os.path.join(path, 'pytorch_model.pt'))
        ...
        for k, v in self.ds_engine.module.named_parameters():
            if v.requires_grad:
                checkpoint[k] = v
            # Cause it doesnt require grad in stage 2, but with still need it for evaluation
            if 'llama_proj' in k:
                checkpoint[k] = v
    ...
    def load_parameters(self, path, stage=3):
        if os.path.exists(os.path.join(path, 'pytorch_model.pt')):
            print('loading parameters from {}'.format(self.args['load_path']))
            delta_ckpt = torch.load(f'{path}/pytorch_model.pt', map_location=torch.device('cuda'))
            #checkpoint = OrderedDict()
            if False:#stage == 2: # from no LoRA to LoRA
