...
def process_and_append_to_csv(target_ids, anchor_ids, distractor_ids, utterances, stimulus_id, csv_file_path):
    """
    Processes the given data and appends it to a CSV file.

    Args:
    target_ids (torch.Tensor): The tensor of target IDs.
    anchor_ids (list of list of torch.Tensor): The list of anchor IDs for each batch.
    distractor_ids (list of list of torch.Tensor): The list of distractor IDs for each batch.
    utterances (list of str): The list of utterances to process.
